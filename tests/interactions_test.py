...
class TestInteractions:
    ...
    class TestSortablePage:
        ...
        def test_sortable(self, driver):
            ...
            ), "The order of the list has not been changed"
            assert (
    ...
    class TestDroppablePage:
        ...
        def test_revert_draggable_droppable(self, driver):
            ...
            (
                not_will_after_move,
                not_will_after_revert,
            ) = droppable_page.drop_revert_draggable("not_will")
    ...
    class TestDraggablePage:
        ...
        def test_axis_restricted_draggable(self, driver):
            ...
            assert (
                top_x[0][0] == top_x[1][0] and int(top_x[1][0]) == 0
            ), "box position has not changed or there has been a shift in the y-axis"
            assert (
                left_x[0][0] != left_x[1][0] and int(left_x[1][0]) != 0
            ), "box position has not changed or there has been a shift in the y-axis"
            assert (
                top_y[0][0] != top_y[1][0] and int(top_y[1][0]) != 0
            ), "box position has not changed or there has been a shift in the x-axis"
            assert (
                left_y[0][0] == left_y[1][0] and int(left_y[1][0]) == 0
            ), "box position has not changed or there has been a shift in the x-axis"
